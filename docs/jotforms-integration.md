...
We have now moved out Next Web JotForms integration to use Storyblok.  This replaces the need to create pages manually under the `/forms` directory.
...
Routing for forms is now managed by Storyblok and our dynamic routing for corporate content pages.


## Storyblok components for form pages

<!-- TODO: complete docs for SB FormPage in NXT-839 -->
### FormPage
...
**FormPage field descriptions**
- header - allows content editors to a PageHeader block to the FormPage, the PageHeader includes the following fields.
	- title - Used as the H1 for the form page, also used to populate the page title for SEO
	- summary - lead text that supports the page title and context
	- description - further supporting content for the form page
	- CTA (Call to action) - Provides a button link to link to supporting content, if required  
	- theme - provides 2 options to theme PageHeader.
More information on the page header can be found in our [design system documentation](https://design-system.nice.org.uk/components/page-header/).

- metadata - allows a limited set of fields to populate SEO data for the form page. metadata includes the following fields:
    - Description - Allows content editors to add a meta description to the form page.  
	- Creator (optional) - Adds a [DC.Creator](https://www.microfocus.com/documentation/content-manager/23.4/ContentManager_en/Content/trim_dublin_core_elements.html?TocPath=Content%20Manager%20Help%7CWeb%20publisher%7CWeb%20publisher%20layouts%20administration%7CCreating%20Web%20publisher%20layouts%7CMetadata%20tab%7C_____1#:~:text=DC.,X.500) meta tag to the form page 

- content - Includes the following blocks:
	- FormEmbed - Allows editors to add the JotForm form embed to the form page, See the [FormEmbed](#formembed) for more information.
	- NestedRichText - Allows editors to add supporting text before and after the form embed.
	- ButtonLink - Allows editors to add a link styled as button to the form page, allows editors to link to supporting content.
	- Panel - An optional side panel that displays supporting information or guidance for the form on page. 
