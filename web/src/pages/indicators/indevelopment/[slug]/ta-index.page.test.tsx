...
describe("/indevelopment/[slug].page", () => {
	...
	describe("IndevelopmentPage", () => {
		...
		it("should render the page title with reversed breadcrumbs for SEO", async () => {
			...
			await waitFor(() => {
				expect(document.title).toEqual(
					`Rapid MT-RNR1 testing for detecting a genetic variant to guide antibiotic use and prevent hearing loss in babies (provisional title) | Indicators`
				);
			});
		});
		...
		it("should render a non-linked breadcrumb for the project id", () => {
			...
			const idBreadcrumb = within(navigation).getByText("GID-TA12552");
			expect(idBreadcrumb).toBeInTheDocument();

			expect(
				screen.queryByRole("link", { name: "GID-TA12552" })
		...
		describe("InfoAlert", () => {
			it("should not appear when alert is null or undefined", () => {
				...
				render(<InDevelopmentPage {...propsWithoutAlert} />);
		...
		it("should render email enquiries mailto link", () => {
			...
			const link = screen.getByRole("link", {
				name: "HealthTech2@nice.org.uk",
			});

			expect(link).toBeInTheDocument();

			expect(link).toHaveAttribute("href", "mailto:HealthTech2@nice.org.uk");
		});
