...
export const getNextPublicEnvVars = async (): Promise<EnvVarsType> => {
	if (typeof window === "undefined") {
		...
		return {
			...
			PUBLIC_AUTH_ENVIRONMENT:
				(process.env.PUBLIC_AUTH_ENVIRONMENT as
					| "test"
					| "live"
					| "beta"
					| "local") || "local",
			...
			PUBLIC_BUILD_NUMBER: process.env.PUBLIC_BUILD_NUMBER || "0",
	...
	const response = await fetch(`${process.env.PUBLIC_BASE_URL || ""}/api/env`, {
		cache: "no-store",
	});
