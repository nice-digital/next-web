Feature: Users can page through the list of published recommendation for research page
	As a user of published recommendation for research page
	We want to be able to page through the list of published recommendation for research page

	Background:
  	Given I open the home page
    # Given I have a screen that is 1400 by 900 pixels

	Scenario: User can page through the list of published recommendation for research page
    Given I open the recommendation for research list page
    Then I expect the first pagination option is "1"
    # When I click the second pagination option
		# Then I expect the first pagination option is "Previous page"
		# And I click the next page option
		# And I click the previous page option
		# And I click the previous page option
		# Then I expect the first pagination option is "1"
